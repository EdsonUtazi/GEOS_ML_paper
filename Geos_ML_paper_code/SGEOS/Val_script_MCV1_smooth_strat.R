...
gg <- 10 #No of groups tuned to avoid numerical errors in inla functions
vaxcov.all$l_n2_Mean_gp <- inla.group(vaxcov.all$l_n2_Mean, n = 5, method = "quantile")# , method = "quantile")
vaxcov.all$n26_Mean_gp <- inla.group(vaxcov.all$n26_Mean, n = 10, method = "quantile") #10 chosen because range of values is small
vaxcov.all$l_n28_Mean_gp <- inla.group(vaxcov.all$l_n28_Mean, n = 5, method = "quantile") #NOTE
vaxcov.all$n12_Mean_gp <- inla.group(vaxcov.all$n12_Mean, n = 10, method = "quantile")
vaxcov.all$l_n51_Mean_gp <- inla.group(vaxcov.all$l_n51_Mean, n = 10, method = "quantile")
...
#Spatial stratification - data arrangement is already spatial
...
for (kk in 1:cv){
  ...
  xp2     <- vaxcov$l_n58_Mean
  xp3     <- vaxcov$n69_Mean
  xp4     <- vaxcov$logit_phone_internet.prop
  xp5     <- vaxcov$media.prop
  xp6     <- vaxcov$l_n2_Mean_gp
  xp7	    <- vaxcov$n26_Mean_gp
  xp8     <- vaxcov$l_n28_Mean_gp
  xp9	    <- vaxcov$n12_Mean_gp
  xp10	    <- vaxcov$l_n51_Mean_gp
  xp11     <- vaxcov$logit_wealth.prop_gp
  xp12     <- vaxcov$logit_educ.prop_gp
  xp13     <- vaxcov$health_card_doc.prop_gp
  xp14     <- vaxcov$logit_sba.prop_gp
  
  #Covariates for validation
  xv1     <- vaxcov.nc$URBAN_RURA
  xv2     <- vaxcov.nc$l_n58_Mean
  xv3     <- vaxcov.nc$n69_Mean
  xv4     <- vaxcov.nc$logit_phone_internet.prop
  xv5     <- vaxcov.nc$media.prop
  xv6     <- vaxcov.nc$l_n2_Mean_gp
  xv7	    <- vaxcov.nc$n26_Mean_gp
  xv8     <- vaxcov.nc$l_n28_Mean_gp
  xv9	    <- vaxcov.nc$n12_Mean_gp
  xv10	    <- vaxcov.nc$l_n51_Mean_gp
  xv11     <- vaxcov.nc$logit_wealth.prop_gp
  xv12     <- vaxcov.nc$logit_educ.prop_gp
  xv13     <- vaxcov.nc$health_card_doc.prop_gp
  xv14     <- vaxcov.nc$logit_sba.prop_gp
  ...
  ux6 <- sort(unique(vaxcov.all$l_n2_Mean_gp)); ux7 <- sort(unique(vaxcov.all$n26_Mean_gp))
  ux8 <- sort(unique(vaxcov.all$l_n28_Mean_gp)); ux9 <- sort(unique(vaxcov.all$n12_Mean_gp))
  ux10 <- sort(unique(vaxcov.all$l_n51_Mean_gp)); ux11 <- sort(unique(vaxcov.all$logit_wealth.prop_gp))
  ux12 <- sort(unique(vaxcov.all$logit_educ.prop_gp)); ux13 <- sort(unique(vaxcov.all$health_card_doc.prop_gp))
  ux14 <- sort(unique(vaxcov.all$logit_sba.prop_gp))
  ...
  #fx3hat <- Xx3[idx3,]; fx4hat <- Xx4[idx4,]
  #fx5hat <- Xx5[idx5,]; 
  fx6hat <- Xx6[idx6,]
...
#Write output files
write.csv(val.out, paste0(filePathData2, "val_out_mcv1_smooth_strat.csv"))
write.csv(obspred.out, paste0(filePathData2, "val_obsvpred_mcv1_smooth_strat.csv"))
