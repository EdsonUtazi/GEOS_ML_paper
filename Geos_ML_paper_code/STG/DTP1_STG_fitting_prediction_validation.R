...
#Specify Drive Path
drive_path <- "/.../Working/"
input_path <- paste0(drive_path, "Methods_comparison_paper/.../DTP1/")
data_path <- paste0(drive_path, "Methods_comparison_paper/2018_NGA_DHS/")
output_path <- paste0(drive_path, "Methods_comparison_paper/.../DTP1/Predictions/")
...
dtp1 <- read.csv(paste0(input_path, "gam_data.csv"))
...
dtp1 <- dtp1 %>% 
  select(X, DHSCLUST, prop, DHSCLUST, LONGNUM, LATNUM, Age12_23_Vax, Age12_23_Tot)
...
#Join datasets
dtp1 <- full_join(dtp1, lasso_join, by = "DHSCLUST")
...
#Calculate dtp1 proportion
dtp1 <- dtp1 %>% 
  mutate(prop = Age12_23_Vax/Age12_23_Tot)

# Extract coordinates from your data frame
xy <- dtp1[, c("LONGNUM", "LATNUM")]

# Create the SpatialPointsDataFrame
dtp_sf <- SpatialPointsDataFrame(coords = xy, data = dtp1,
...
cmp <- Age12_23_Vax ~ Intercept(1) + x(cbind(gam_predictions, 
...
res <- bru(components = cmp,  family = "binomial", 
           Ntrials = dtp_sf$Age12_23_Tot, data = dtp_sf,
...
kfold_cv <- function(data, k) {
  ...
  for (i in 1:k) {
    ...
    # Define model components and constraint
    cmp2 <- Age12_23_Vax ~ Intercept(1) + x(cbind(gam_predictions, 
                                                  gbm_predictions, lasso_predictions), 
                                            model = "iid",
                                            mapper = bru_mapper_matrix(labels = c("gam_predictions", "gbm_predictions", "lasso_predictions")),
                                            hyper = list(prec = list(initial = -log(30), fixed = TRUE)), #can be initial = -log(30) or log(1/1000) #-2*log(30)
    ...
    res2 <- bru(components = cmp2,  family = "binomial", 
                Ntrials = train_data$Age12_23_Tot, data = train_data,
                options = list(control.fixed=control.fixed))
...
spatial_kfold_cv <- function(data) {
  ...
  #Calculate prediction coverage using the generated samples
...
r1 <- rast(paste0(input_path, "GAM_cubic.tif"))
...
pred_covs <- pred_covs %>% 
  rename(LONGNUM = x, LATNUM = y,
         ...
         gam_predictions = gam_cubic_predictions) 
...
writeRaster(predictions_raster, paste0(input_path, "STG_dtp1_12_23_mean.tif"), 
            overwrite = T, names = "DTP1_Coverage")
...
writeRaster(sd_raster, paste0(input_path, "STG_dtp1_12_23_sd.tif"), 
            overwrite = T, names = "DTP1_sd")

#Rasterize upper bound
upper_raster <- rasterize(inv.linpred_sf, r1, field = "upper_quantile")
plot(upper_raster)

writeRaster(upper_raster, paste0(input_path, "STG_dtp1_12_23_upper.tif"), 
            overwrite = T, names = "DTP1_upper")
...
writeRaster(lower_raster, paste0(input_path, "STG_dtp1_12_23_lower.tif"), 
            overwrite = T, names = "DTP1_lower")
