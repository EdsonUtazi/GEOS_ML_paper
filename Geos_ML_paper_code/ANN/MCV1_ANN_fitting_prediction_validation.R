...
write.csv(nn_insample_df, paste0(filePathData2, "mcv1_nn_insample_result.csv"))
...
write.csv(rand_prediction_data, paste0(filePathData2, "mcv1_nn_rand_kfold_results.csv"))
...
write.csv(spatial_prediction_data, paste0(filePathData2, "mcv1_nn_spatial_kfold_results.csv"))


kfold_spatial_df <- as.data.frame(data_h2o) %>%
  select(DHSCLUST, LATNUM, LONGNUM, Age9_35_Vax, Age9_35_Tot) %>%
...
# Print the summary metrics
print(summary_metrics)
write.csv(summary_metrics, paste0(filePathData2, "mcv1_summary_metrics.csv"))
...
# Save raster output
writeRaster(DL_raster, paste0(filePathData2, "mcv1_spatial.tif"), overwrite = TRUE, names = "dl_predictions")
