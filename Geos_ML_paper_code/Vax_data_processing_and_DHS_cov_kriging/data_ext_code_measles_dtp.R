...
setwd("/.../Working/DHS_2018_data")

#Load DHS data
data1 <- read.dta("/.../Working/DHS_2018_data/NGKR7ADT/NGKR7AFL.DTA",
...
#Child variables
#hw1 - child's age in months; b1 - month of birth; b2 - year of birth; b3 - date of birth (cmc); b17 - day of birth; b5 - alive
...
vars1 = c("v001", "v006", "v007", "v016", "hw1", "b1", "b2", "b3", "b17", "h9", "h9d", "h9m", "h9y")

vars2 = c("v001", "v006", "v007", "v016", "hw1", "b1", "b2", "b3", "b17", "h3", "h3d", "h3m", "h3y", "h5",
...
#Calculation of child's age in months - Method 1
#Measles
...
#Calculation of child's age in months - Method 2
#Measles
dd.m <- data.m.ext %>% 
...
data.m.ext$age.m2 = dd.m$age_at_int_month
...
dd.D <- data.D.ext %>% 
  mutate(b_date = make_date(b2,b1,b17), 
         ...
         age_at_int_years = floor(age_at_int_days/365.25),
         age_at_int_month = floor(age_at_int_days/30.4375)) 
data.D.ext$age.D2 = dd.D$age_at_int_month
...
write.csv(data.m.file1, "Vax_NGA_measles_proc.csv")
...
received_dtp1=received_dtp2=received_dtp3=numeric(nrow(data.D))
for (i in 1:nrow(data.D)){
  ss <- sum(vacc.num.D1[i]+vacc.num.D2[i]+vacc.num.D3[i])
  if (ss>=1) received_dtp1[i]=1
  if (ss>=2) received_dtp2[i]=1
  if (ss>=3) received_dtp3[i]=1
...
write.csv(data.D, "Vax_NGA_dtp_proc.csv")            
