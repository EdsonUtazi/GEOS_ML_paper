## dtp1 Modelling using XGBoost Regression
...
kfold_cv <- function(data, formula, k = 10) {
  ...
  write.csv(results_df, paste0(output_path, "GBM_Kfold_Predictions.csv"))
...
# Apply function
result1 <- kfold_cv(data = dtp1, formula = logit_prop ~ n26_Mean + n28_Mean + n51_Mean + n58_Mean + n69_Mean + wealth.prop +
                      educ.prop + health_card_doc.prop + sba.prop + phone_internet.prop + 
                      n2_Mean + media.prop + n12_Mean + urban_rural)
...
spatial_kfold_cv <- function(data, formula) {
  ...
  # Write results to CSV
  write.csv(results_df, paste0(output_path, "GBM_Spatial_Kfold_Predictions.csv"))
...
# Apply function
result2 <- spatial_kfold_cv(data = dtp1, formula = logit_prop ~ n26_Mean + n28_Mean + n51_Mean + n58_Mean + n69_Mean + wealth.prop +
                              educ.prop + health_card_doc.prop + sba.prop + phone_internet.prop + 
                              n2_Mean + media.prop + n12_Mean + urban_rural)
...
writeRaster(GBM_raster , paste0(output_path, "GBM.tif"), 
            overwrite = T, names = "gbm_predictions")

###################################################################################################
###################################################################################################
output_path1 <- paste0(drive_path, "Methods_comparison_paper/.../DTP1/JackKnife/")
...
for (kk in 1:nrast) {
  ...
  writeRaster(r1 , paste0(output_path1, "jack_gbm_", kk, ".tif"), 
              overwrite = T)
}

#Calculate jackknife mean and std. dev.
raster_list <-list.files(path=output_path1, pattern="^jack_gbm.*\\.tif$" , all.files=TRUE, full.names=FALSE)
raster_list
...
writeRaster(mean.rast, paste0(output_path1, "GBM_mean_raster_dtp1_", nrast,".tif"), 
            overwrite = T)
writeRaster(sd.rast, paste0(output_path1, "GBM_stddev_raster_dtp1_", nrast,".tif"), 
            overwrite = T)
