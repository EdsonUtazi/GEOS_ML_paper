## mcv1 Modelling using Lasso Regression
...
x <- mcv1 %>% 
  dplyr::select(n26_Mean, n28_Mean, n51_Mean, n58_Mean, n69_Mean, wealth.prop,
                ...
                n2_Mean,media.prop,n69_Mean, n12_Mean, urban_rural, LATNUM, LONGNUM) %>% 
  as.matrix()


#apply mean imputation and convert to matrix
#x <- x %>%
...
#Select response
y <- mcv1 %>% 
  dplyr::select(success = Age9_35_Vax, Age9_35_Tot) %>%   #count = number of success and total = total cases
  mutate(failure = Age9_35_Tot - success) %>% 
  dplyr::select(failure, success) %>% 
  as.matrix()

##################NEW - create a model matrix for x
xx1 <- model.matrix(~ -1 + n26_Mean + n28_Mean + n51_Mean + n58_Mean + n69_Mean + wealth.prop +
                   educ.prop + health_card_doc.prop + sba.prop + phone_internet.prop +
                   n2_Mean + media.prop + n12_Mean + factor(urban_rural) + LATNUM + LONGNUM, data = mcv1)
...
write.csv(lasso_data, paste0(output_path, "spatial_lasso_data.csv"))
...
kfold_cv <- function(data, k = 10) {
  ...
  for (i in 1:k) {
    ...
    y <- train_data %>% 
      dplyr::select(success = Age9_35_Vax, Age9_35_Tot) %>%   
      mutate(failure = Age9_35_Tot - success) %>% 
...
#      as.matrix()
    ...
    xx1 <- model.matrix(~ -1 + n26_Mean + n28_Mean + n51_Mean + n58_Mean + n69_Mean + wealth.prop +
                          educ.prop + health_card_doc.prop + sba.prop + phone_internet.prop +
                          n2_Mean + media.prop + n12_Mean + factor(urban_rural) + LATNUM + LONGNUM, data = train_data)
...
#      as.matrix()
    ...
    xx2 <- model.matrix(~ -1 + n26_Mean + n28_Mean + n51_Mean + n58_Mean + n69_Mean + wealth.prop +
                          educ.prop + health_card_doc.prop + sba.prop + phone_internet.prop +
                          n2_Mean + media.prop + n12_Mean + factor(urban_rural) + LATNUM + LONGNUM, data = test_data)
  ...
  write.csv(results_df, paste0(output_path, "Spatial_Lasso_Kfold_Predictions.csv"))
...
#      as.matrix()
  ...
  # Write results to CSV
  write.csv(results_df, paste0(output_path, "Spatial_Lasso_Spatial_Kfold_Predictions.csv"))
...
# Apply function
result2 <- spatial_kfold_cv(data = mcv1)
result2
...
#ADD COORDINATES FOR PREDICTION
#Read raster and get xy values
r1 <- rast(paste0(data_path, "n28_viirs_2016_nightlights.tif"))
...
coord <- xyFromCell(r1, 1:ncell(r1))
xxb$LATNUM <- coord[,2]
xxb$LONGNUM <- coord[,1] #Note - latnum before longnum
...
writeRaster(r1 , paste0(output_path, "spatial_lasso.tif"), 
            overwrite = T, names = "spatial_lasso_predictions")
...
#writeRaster(lasso_raster , paste0(output_path, "spatial_lasso.tif"), 
#            overwrite = T, names = "spatial_lasso_predictions")
...
#####################################################################################
###################################################################################
output_path <- paste0(drive_path, "Methods_comparison_paper/.../MCV1/JackKnife/")
...
for (kk in 1:nrast) {
  ...
  y <- mcv1 %>%
    dplyr::select(success = Age9_35_Vax, Age9_35_Tot) %>%  # Success and total counts
    mutate(failure = Age9_35_Tot - success) %>%           # Calculate failures
  ...
  xx1 <- model.matrix(~ -1 +  # Intercept
                        ...
                        n2_Mean + media.prop + n12_Mean + factor(urban_rural)+ LATNUM + LONGNUM,  # Add your explanatory variables here
                      data = mcv1)
  ...
  writeRaster(r1 , paste0(output_path, "spatial_jack_lasso_", kk, ".tif"), 
              overwrite = T)
}

#Calculate jackknife mean and std. dev.
raster_list <-list.files(path=output_path,pattern="spatial_jack_lasso.*\\.tif$" , all.files=TRUE, full.names=FALSE)
raster_list
...
writeRaster(mean.rast, paste0(output_path, "Spatial_LASSO_mean_raster_mcv1_", nrast,".tif"), 
            overwrite = T)
writeRaster(sd.rast, paste0(output_path, "Spatial_LASSO_stddev_raster_mcv1_", nrast,".tif"), 
            overwrite = T)
