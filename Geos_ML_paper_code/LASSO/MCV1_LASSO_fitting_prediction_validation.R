...
xx1 <- model.matrix(~ -1 + n26_Mean + n28_Mean + n51_Mean + n58_Mean + n69_Mean + wealth.prop +
                   educ.prop + health_card_doc.prop + sba.prop + phone_internet.prop +
                   n2_Mean + media.prop + n12_Mean + factor(urban_rural), data = mcv1)
...
raster_list <-list.files(path=output_path,pattern="^jack_lasso.*\\.tif$" , all.files=TRUE, full.names=FALSE)
raster_list
...
writeRaster(mean.rast, paste0(output_path, "LASSO_mean_raster_mcv1_", nrast,".tif"), 
            overwrite = T)
writeRaster(sd.rast, paste0(output_path, "LASSO_stddev_raster_mcv1_", nrast,".tif"), 
            overwrite = T)
