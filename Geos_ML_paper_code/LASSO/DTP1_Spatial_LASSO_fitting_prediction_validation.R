...
xx1 <- model.matrix(~ -1 + n26_Mean + n28_Mean + n51_Mean + n58_Mean + n69_Mean + wealth.prop +
                      educ.prop + health_card_doc.prop + sba.prop + phone_internet.prop +
                      n2_Mean + media.prop + n12_Mean + factor(urban_rural) + LATNUM + LONGNUM, data = dtp1)
...
spatial_kfold_cv <- function(data) {
  ...
  for (i in 1:k) {
    ...
    #Model
    model <-  glmnet::glmnet(y=y, x=xx, family = "binomial", alpha = 1, lambda = best_lambda, intercept = TRUE)
...
writeRaster(mean.rast, paste0(output_path, "Spatial_LASSO_mean_raster_dtp1_", nrast,".tif"), 
            overwrite = T)
writeRaster(sd.rast, paste0(output_path, "Spatial_LASSO_stddev_raster_dtp1_", nrast,".tif"), 
            overwrite = T)
