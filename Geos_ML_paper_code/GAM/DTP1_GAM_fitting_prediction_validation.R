#Generalize Additive Model (GAM) using cubic splines
...
dtp1 <- dtp1 %>% 
  ...
  filter(Age12_23_Tot > 1) %>%    #Remove observations where total is not 0
...
#fit GAM model
fit_GAM <- mgcv::gam(
  formula = Age12_23_Vax/Age12_23_Tot ~ s(n26_Mean, bs = "cs")  + s(n28_Mean, bs = "cs") + s(n51_Mean, bs = "cs") +
    ...
    s(n2_Mean, bs = "cs") + s(media.prop, bs = "cs") +s(n12_Mean, bs = "cs")+ urban_rural,
  family = binomial("logit"),
  weights = Age12_23_Tot,
  data = dtp1)
...
covs1 <- dtp1 %>% 
  select(n26_Mean, n28_Mean, n51_Mean, n58_Mean, n69_Mean, wealth.prop,
         ...
         n2_Mean,media.prop,n69_Mean, n12_Mean, urban_rural)
...
write.csv(gam_data, paste0(output_path, "gam_cubic_data.csv"))
...
kfold_cv <- function(data, formula, k = 10) {
  ...
  for (i in 1:k) {
    ...
    #Model
    model <- mgcv::gam(formula, data = train_data, weights = Age12_23_Tot, family = binomial("logit"))
  ...
  write.csv(results_df, paste0(output_path, "Gam_Cubic_Kfold_Predictions.csv"))
...
# Apply function
result1 <- kfold_cv(data = dtp1,  formula = Age12_23_Vax/Age12_23_Tot ~ s(n26_Mean, bs = "cs")  + s(n28_Mean, bs = "cs") + s(n51_Mean, bs = "cs") +
                      s(n58_Mean, bs = "cs") + s(n69_Mean, bs = "cs")  + s(wealth.prop, bs = "cs") + s(educ.prop, bs = "cs") +
                      s(health_card_doc.prop, bs = "cs") + s(sba.prop, bs = "cs") + s(phone_internet.prop, bs = "cs")  + 
                      s(n2_Mean, bs = "cs") + s(media.prop, bs = "cs") +s(n12_Mean, bs = "cs")+ urban_rural)
...
spatial_kfold_cv <- function(data, formula) {
  ...
  for (i in 1:k) {
    ...
    #Model
    model <- mgcv::gam(formula, data = train_data, weights = Age12_23_Tot,
                       family = binomial("logit"))
  ...
  # Write results to CSV
  write.csv(results_df, paste0(output_path, "Gam_Cubic_Spatial_Kfold_Predictions.csv"))
...
# Apply function
result2 <- spatial_kfold_cv(data = dtp1, formula = Age12_23_Vax/Age12_23_Tot ~ s(n26_Mean, bs = "cs")  + s(n28_Mean, bs = "cs") + s(n51_Mean, bs = "cs") +
                              s(n58_Mean, bs = "cs") + s(n69_Mean, bs = "cs")  + s(wealth.prop, bs = "cs") + s(educ.prop, bs = "cs") +
                              s(health_card_doc.prop, bs = "cs") + s(sba.prop, bs = "cs") + s(phone_internet.prop, bs = "cs")  + 
                              s(n2_Mean, bs = "cs") + s(media.prop, bs = "cs") +s(n12_Mean, bs = "cs")+ urban_rural)
result2
...
#Make predictions using the fitted model
raster_predictions <- mgcv::predict.gam(object = fit_GAM,  newdata = pred_covs, type = "response")
raster_predictions <- as_tibble(raster_predictions) %>% 
...
writeRaster(GAM_raster , paste0(output_path, "GAM_cubic.tif"), 
            overwrite = T, names = "gam_cubic_predictions")
